  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-900">{t("cart.title")} ({items.length} {t("common.lines")})</h1>
        <button onClick={clear} className="text-sm text-red-600 hover:underline">{t("cart.clear")}</button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-start text-xs text-slate-500 border-b border-slate-200">
              <th className="px-4 py-3">{t("cart.part")}</th>
          ...
          <tbody>
            {items.map((i) => (
              <tr key={i.productId} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-3">
                  <button onClick={() => remove(i.productId)} className="text-slate-400 hover:text-red-600" aria-label={t("common.remove")}>
      ...
      <div className="mt-6 flex justify-end">
        <div className="bg-white rounded-lg border border-slate-200 p-5 w-full sm:w-80">
          ...
          <RequestQuoteButton />
          <div className="text-[11px] text-slate-400 text-center mt-2">{t("cart.guestNote")}</div>
        </div>
      </div>
    </div>
  );
