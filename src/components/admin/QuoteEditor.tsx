...
export default function QuoteEditor({
  ...
  const send = async () => {
    ...
    else alert(t("admin.sendQuoteFailed"));
  };
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-start text-xs text-slate-500 border-b border-slate-200">
              ...
              <th className="px-4 py-3">{t("admin.list")}</th>
              <th className="px-4 py-3">{t("admin.quotedUsd")}</th>
          ...
          <tbody>
            {items.map((it) => (
              <tr key={it.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-2.5 text-xs text-slate-500">{it.stockQty} {t("common.avail")}</td>
      ...
      <div className="mt-3 text-sm text-slate-600 text-end">
        {t("admin.list")}: <span className="line-through">{formatPrice(listTotal)}</span> · {t("quotes.quotedPrice")}:{" "}
        <strong>{formatPrice(quotedTotal)}</strong>{" "}
        <span className="text-slate-400">
          ({listTotal > 0 ? Math.round((1 - quotedTotal / listTotal) * 100) : 0}% {t("admin.off")})
        </span>
      </div>
